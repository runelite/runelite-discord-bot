  ...
  switch (command) {
    case 'help': {
      const helpEmbed = createEmbed()
        ...
        .addField('!ghsearch <query>', 'Search runelite/runelite GitHub repository for file')
    ...
    case 'gh': {
      ...
      if (!issueBody.items || issueBody.items.length === 0) {
        return message.channel.send(`Search for **${value}** returned no results.`)
      }
    ...
    case 'ghsearch': {
      const codeBody = await fetch(`https://api.github.com/search/code?q=repo:runelite/runelite+filename:${value}`, {
        headers: {
          Authorization: `token ${config.githubToken}`
        }
      }).then(res => res.json())

      if (!codeBody.items || codeBody.items.length === 0) {
        return message.channel.send(`Search for **${value}** returned no results.`)
      }

      const item = codeBody.items[0]
      return message.channel.send(`**${item.name}** <${item.html_url}>`)
    }
    case 'add': {
