...
export function findCommand (name) {
  if (name in commandCache) {
    return commandCache[name]
  }

  for (const command of Object.values(commandCache)) {
...
export const initCommands = async (applicationId, guild) => {
  ...
  await Promise.all(response.map(async c => {
    const commandDefinition = findCommand(c.name)
...
export const deleteCommand = async (applicationId, guild, command) => {
  ...
  const commandDefinition = guild.commands.cache.find(c => c.name.toLowerCase() === command.toLowerCase())
