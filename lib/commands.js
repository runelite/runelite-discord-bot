    ...
    case 'ghwhois': {
      ...
      if (mention) {
        ...
        if (ghId) {
          ...
          let embed = createEmbed().setDescription(`${value} is not a registered GitHub user.`)

          if (userResponse.id) {
            embed = createEmbed()
              .setAuthor(userResponse.login, userResponse.avatar_url, userResponse.html_url)
              .setThumbnail('')
          }
      ...
      let embed = createEmbed().setDescription(`${value} has not connected their Discord account.`)
