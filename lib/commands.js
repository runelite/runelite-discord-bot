...
const { log, createEmbed, sendDM, githubUserDb } = require('./common')
          ...
          if (body.id) {
            ...
            message.client.guilds.forEach(g => g
              ...
              .then(m => {
                ...
                // Add contributor roles to user if they've contributed
                if (contributors[body.id]) {
                  for (let roleToGive in contributors[body.id]) {
                    const role = g.roles.find(r => r.name.toLowerCase() === roleToGive.toLowerCase())
                    m.addRole(role).catch(e => log.debug(e))
                  }
                }
              })
