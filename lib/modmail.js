...
function processModMailTyping (typing, guilds) {
  async function processTyping (typing, guild) {
    ...
    const channel = guild.channels.cache.find(c => c.name === config.channels.support)
    ...
    userCooldowns.set(member.id, now + userCooldown)
    await sendDM(member, `After sending message here support thread will be automatically created for you. Moderators will then see it and reply. **Mod mail is only for questions and support related to Discord, for client help use <#${channel.id}> channel.**`)
  }
