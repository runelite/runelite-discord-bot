...
export default {
  ...
  async execute (interaction) {
    switch (interaction.options.getSubcommand()) {
      case 'show':
        if (!notice.isEnabled()) {
          return interaction.followUp({ content: 'Notices must have a description and title set', ephemeral: true })
        }

        return interaction.followUp({ embeds: [notice.embed], ephemeral: true })
      case 'on':
        notice.enabled = true
    ...
    return interaction.followUp({ content: `\`${interaction.options.getSubcommand()}\` updated.`, ephemeral: true })
  }
}
