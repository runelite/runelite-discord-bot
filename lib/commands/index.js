...
export function allCommands () {
  const customCommands = commandsDb.ls().sort().map(c => ({
    name: c,
    data: new SlashCommandBuilder().setName(c).setDescription('Custom command'),
    async execute (interaction) {
      return interaction.reply(commandsDb.get(c))
    }
...
export const addCommand = async (applicationId, guild, command) => {
  ...
  commandList[guild.id] = commandList[guild.id] + [response]
}
