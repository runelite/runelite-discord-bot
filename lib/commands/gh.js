...
export default {
  ...
  async execute (interaction) {
    ...
    const fileSearch = await fetch(`https://api.github.com/search/code?q=repo:runelite/runelite+filename:${value}`, {
      headers: {
    ...
    const issueSearch = await fetch(`https://api.github.com/search/issues?q=repo:runelite/runelite+${value}`, {
      headers: {
        Authorization: `token ${config.githubToken}`
      }
    }).then(res => res.json())
