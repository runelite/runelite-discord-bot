...
export default {
  ...
  async execute (interaction) {
    ...
    if (user) {
      ...
      let content = `\`${user.displayName}\` does not have Discord and GitHub accounts linked.`
      ...
      if (ghId) {
        ...
        if (userResponse.id) {
          content = `[ <${userResponse.html_url}> ] ${buildUserDetail(user.user)}`
        } else {
          content = `\`${user.displayName}\` is not a registered GitHub user.`
        }
      }
        ...
        if (repoSplit.length >= 3) {
          ...
          if (userResponse.id) {
            ...
            if (discordId) {
              ...
              if (messMember) {
                content = `[ <${userResponse.html_url}> ] ${buildUserDetail(messMember.user)}`
              }
            } else {
