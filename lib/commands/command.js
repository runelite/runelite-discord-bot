...
import { limitStrTo } from '../common.js'
  ...
  async execute (interaction, applicationId) {
    switch (interaction.options.getSubcommand()) {
      case 'add': {
        ...
        await addCommand(applicationId, interaction.guild, new SlashCommandBuilder()
          .setName(name)
          .setDescription(limitStrTo(content, 50)))
      ...
      case 'ls': {
        return interaction.followUp('Commands:\n`' + commandsDb.ls().sort().join('`\n`') + '`')
      }
    }
  }
}
