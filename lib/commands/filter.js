  ...
  async execute (interaction) {
    switch (interaction.options.getSubcommand()) {
      case 'add': {
        ...
        return interaction.editReply({ content: `Successfully added filter \`${value}\``, ephemeral: true })
      }
      case 'del': {
        const value = interaction.options.getString('regex')
        filteredWordsDb.put(value)
        return interaction.editReply({ content: `Successfully removed filter \`${value}\``, ephemeral: true })
      }
      case 'ls': {
