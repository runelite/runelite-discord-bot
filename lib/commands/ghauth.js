...
export default {
  ...
  async execute (interaction) {
    ...
    if (!value) {
      return interaction.reply({ content: `Please visit <https://github.com/login/oauth/authorize?client_id=${config.github.clientId}> and follow the instructions.`, ephemeral: true })
    }

    // GitHub's OAuth codes are 20 characters long
    if (value.length !== 20) {
      return interaction.reply({ content: 'The supplied token is invalid.', ephemeral: true })
    }
    ...
    if (!userResponse.id) {
      return interaction.reply({ content: 'The supplied token expired.', ephemeral: true })
    }

    const authedDiscordUserId = githubUserDb.get(userResponse.id)

    if (blocked[userResponse.id]) {
      return interaction.reply({ content: 'Your account has been banned from using this feature.', ephemeral: true })
    }
    ...
    return interaction.reply({ content: 'You have successfully linked your GitHub account with the bot.', ephemeral: true })
  }
}
