...
export default {
  ...
  async execute (interaction) {
    ...
    if (!value) {
      return interaction.editReply({ content: `Please visit <https://github.com/login/oauth/authorize?client_id=${config.github.clientId}> and follow the instructions.`, ephemeral: true })
    }

    // GitHub's OAuth codes are 20 characters long
    if (value.length !== 20) {
      return interaction.editReply({ content: 'The supplied token is invalid.', ephemeral: true })
    }
    ...
    if (!userResponse.id) {
      return interaction.editReply({ content: 'The supplied token expired.', ephemeral: true })
    }

    const authedDiscordUserId = githubUserDb.get(userResponse.id)

    if (blocked[userResponse.id]) {
      return interaction.editReply({ content: 'Your account has been banned from using this feature.', ephemeral: true })
    }
    ...
    return interaction.editReply({ content: 'You have successfully linked your GitHub account with the bot.', ephemeral: true })
  }
}
