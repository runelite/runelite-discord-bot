...
async function onMessageDelete (message) {
  ...
  await logs.send({
    content: `:wastebasket: ${buildUserDetail(user)}'s message was deleted${extraMessage}.${messageDetail}`,
...
client.on('guildMemberUpdate', async (oldMember, newMember) => {
  ...
  if (oldMember.roles.cache.size > newMember.roles.cache.size) {
    ...
    await modLogs.send(`:timer: ${buildUserDetail(user)} ${timeoutMessage}${executorMessage}.${reasonMessage}`, { allowedMentions: { users: [] } })
  }
})
