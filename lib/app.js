...
import { allCommands, findCommand, initCommands } from './commands.js'
...
client.on('interactionCreate', async interaction => {
  ...
  const command = findCommand(interaction.commandName, commands)
...
client.on('guildMemberUpdate', async (oldMember, newMember) => {
  ...
  if (oldMember.roles.cache.size > newMember.roles.cache.size) {
    ...
    const roleStr = auditEntry.changes.flatMap(c => c.new).map(c => `**${c.name}**`).join(', ')
