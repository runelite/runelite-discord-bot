...
const { log, fetchAuditEntryFor } = require('./common')
...
const { proccessFilters, resetMessageCache, buildUserDetail, buildMessageDetail, isOurDeletion, pruneDeletedMessages } = require('./security')
const { flagMessageIfNeeded, pruneRecentlyFlagged } = require('./flag')
...
client.on('ready', () => {
  ...
  scheduleWithFixedDelay(client, () => pruneRecentlyFlagged(), 5 * 60000)
...
client.on('message', message => {
  if (!proccessFilters(message, client)) {
    return
  }
...
client.on('messageUpdate', (oldMessage, newMessage) => {
  proccessFilters(newMessage, client)
})
