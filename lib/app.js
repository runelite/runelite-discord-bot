  ...
  fetch(`https://api.twitch.tv/kraken/streams?channel=${streamerId}`, {
    ...
    .then(body => {
      ...
      const embed = createEmbed()
        .setColor(6570406)
        .setDescription(`${newMember.toString()} ${stream.channel.status}`)
        .setThumbnail(stream.channel.logo)
        .setAuthor(`${stream.channel.display_name} is now Live on Twitch!`, newMember.user.displayAvatarURL, stream.channel.url)
        .setImage(image)
