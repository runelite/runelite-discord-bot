...
import { allCommands, initCommands } from './commands.js'
...
client.on('guildMemberUpdate', async (oldMember, newMember) => {
  ...
  if (oldMember.roles.cache.size > newMember.roles.cache.size) {
    ...
    await logs.send(`:person_running: ${buildUserDetail(user)}'s roles were updated. Roles removed: ${roleStr}.`, { allowedMentions: { users: [] } })
    ...
    const roleStr = roles.map(r => `**${r}**`).join(', ')
    await logs.send(`:person_doing_cartwheel: ${buildUserDetail(user)}'s roles were updated. Roles added: ${roleStr}.`, { allowedMentions: { users: [] } })
  }
})
