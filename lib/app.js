...
function fetchAllContributors (client) {
  let contributorPromise = fetchContributors(config.pluginHubRepo, config.github.pluginHubContributedRole, client.guilds, 0)

  for (let contributorRepo of config.contributorRepos) {
    const contrCallback = () => fetchContributors(contributorRepo, config.github.contributedRole, client.guilds, 0)
