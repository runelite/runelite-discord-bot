...
function spamFilter (message, client) {
  ...
  if (spamMatches.length >= config.spam.kickThreshold) {
    ...
    const reason = `Spammed ${spamMatches.length} messages in a row.` + buildMessageDetail(message)
    member.kick(reason)
  ...
  if (spamMatches.length >= config.spam.muteThreshold) {
    ...
    return `:mute: Muted. Spammed ${spamMatches.length} messages in a row.` + buildMessageDetail(message)
  }
  ...
  if (duplicateMatches.length >= config.spam.maxDuplicatesKick) {
    ...
    const reason = `Spammed ${duplicateMatches.length} same messages in a row.` + buildMessageDetail(message)
    member.kick(reason)
    return `:foot: Kicked. ${reason}`
  }
  ...
  if (duplicateMatches.length >= config.spam.maxDuplicatesMute) {
    deleteMessages([...duplicateMatches, ...spamOtherDuplicates], client)
    assignRole(member, message.guild.roles, config.roles.muted)
    return `:mute: Muted. Spammed ${duplicateMatches.length} same messages in a row.` + buildMessageDetail(message)
  }
