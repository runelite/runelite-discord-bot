...
function processFilters (message, client, onlyContent = false) {
  try {
    const filteredResult = messageFilter(message, client, onlyContent)
    ...
    logs.send(`${buildUserDetail(message.author)}'s ${messageDetail}`)
      .then(() => filteredResult.dm && sendDM(message.author, `Your ${messageDetail}`))
      .catch(log.debug)
  } catch (e) {
    log.debug(e)
  }
...
function filterResponse (reason, recentlyDeletedMessage, dm = true) {
  return { reason, log: !recentlyDeletedMessage, dm }
}
...
function contentFilter (message) {
  if (message.attachments.size > 0) {
    if (!(message.attachments.size === message.attachments.filter(a => {
      ...
      return filterResponse(':paperclip: Filtered attachment type', deleteMessage(message))
    }
  }
  ...
  if (filteredWord) {
    return filterResponse(`:speak_no_evil: Filtered word: \`${filteredWord}\``, deleteMessage(message))
  }
...
function spamFilter (message, client) {
  ...
  if (spamMatches.length >= config.spam.kickThreshold) {
    ...
    member.kick(reason)
    return filterResponse(`:foot: Kicked. ${reason}`, recentlyDeleted, false)
  }

  if (spamMatches.length >= config.spam.muteThreshold) {
    const recentlyDeleted = deleteMessages(spamMatches, client)
    assignRole(member, message.guild.roles, config.roles.muted)
    return filterResponse(`:mute: Muted. Spammed ${spamMatches.length} messages in a row`, recentlyDeleted, false)
  }
  ...
  if (spamMatches.length >= config.spam.banThreshold) {
    ...
    const reason = `Spammed ${spamMatches.length} messages in a row`
    member.ban(reason)
    return filterResponse(`:hammer: Banned. ${reason}`, recentlyDeleted, false)
  }
  ...
  if (duplicateMatches.length >= config.spam.maxDuplicatesMute) {
    const recentlyDeleted = deleteMessages([...duplicateMatches, ...spamOtherDuplicates], client)
    assignRole(member, message.guild.roles, config.roles.muted)
    return filterResponse(`:mute: Muted. Spammed ${duplicateMatches.length} same messages in a row`, recentlyDeleted, false)
  }
...
function messageFilter (message, client, onlyContent) {
  ...
  const contentMsg = contentFilter(message)
