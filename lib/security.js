...
function ensureRoles (member, allRoles) {
  const roles = roleDb.get(member.id)

  if (roles) {
    for (const role of roles) {
      assignRole(member, allRoles, role)
    }
  }
}
...
function assignRole (member, allRoles, roleToGive) {
  ...
  const hasSkippedRole = !config.roles.roleAssignmentIgnore.includes(roleToGive) &&
    member.roles.cache.find(r => config.roles.roleAssignmentIgnore.includes(r.name.toLowerCase()))

  if (!hasSkippedRole && !foundRole) {
    const role = allRoles.cache.find(r => r.name.toLowerCase() === roleName)
    if (role) {
      if (config.roles.roleAssignmentPersist.includes(roleToGive)) {
        const roles = roleDb.get(member.id) || []
        roles.push(roleToGive)
        roleDb.put(member.id, [...new Set(roles)])
      }
...
function removeRole (member, allRoles, roleToRemove) {
  ...
  if (foundRole) {
    const role = allRoles.cache.find(r => r.name.toLowerCase() === roleName)
    if (role) {
      if (config.roles.roleAssignmentPersist.includes(roleToRemove)) {
        let roles = roleDb.get(member.id) || []
