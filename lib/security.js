...
function processFilters (message, client, isEdit = false) {
  try {
    ...
    logs.send({
      content: withoutMentions(`${buildUserDetail(message.author)}'s ${messageDetail}`),
      files: message.attachments.filter(isAttachmentOk).map(a => a)
...
function messageFilter (message, client, isEdit) {
  ...
  if (filteredWord) {
    return filterResponse(`:speak_no_evil: Filtered expression: \`${filteredWord}\``, deleteMessage(message), true)
  }
