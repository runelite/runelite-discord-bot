...
function filterResponse (reason, recentlyDeletedMessage, dm = false) {
  return { reason, log: !recentlyDeletedMessage, dm }
}

function messageContentFilter (messageContent) {
  return config.spam.filteredWords.find(w => messageContent.includes(w)) ||
    filteredWordsDb.ls().find(w => messageContent.match(new RegExp(w, 'g')))
}

function spamFilter (message, client, contentOnly) {
  if (message.attachments.size > 0) {
    if (!(message.attachments.size === message.attachments.filter(a => {
      ...
      return filterResponse(':paperclip: Filtered attachment type', deleteMessage(message), true)
    }
  }
  ...
  const currentMessage = {
    ...
    content: message.content.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ''),
  ...
  if (bannedWordMatches.length >= config.spam.kickThreshold) {
    ...
    const reason = `Used ${bannedWordMatches.length} banned words in a row`
    member.kick(reason)
  ...
  if (bannedWordMatches.length >= config.spam.muteThreshold) {
    ...
    return filterResponse(`:mute: Muted. Used ${bannedWordMatches.length} banned words in a row`, recentlyDeleted)
  }
  ...
  if (filteredWord) {
    return filterResponse(`:speak_no_evil: Filtered word: \`${filteredWord}\``, deleteMessage(message), true)
  }
