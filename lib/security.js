...
function messageFilter (message, client, isEdit) {
  ...
  const spammedChannelMessages = cachedMessages.filter((m) => m.sentTimestamp > (currentMessage.sentTimestamp - config.spam.maxChannelHopInterval))
  const channelMatches = multipleChannels(spammedChannelMessages)
  ...
  if (channelMatches >= config.spam.kickThreshold) {
    const recentlyDeleted = deleteMessages(spammedChannelMessages, client)
    const reason = `Posted in ${channelMatches} channels too quickly, likely spam`
    member.kick(reason)
...
function multipleChannels (recentMessages) {
  let channelVal = 0
  const channels = []
  for (const msg of recentMessages) {
    if (channels.indexOf(msg.channelID) === -1) {
      channelVal = channels.push(msg.channelID)
    }
  }
  return channelVal
}
